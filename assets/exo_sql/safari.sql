...
/* Story 2 */
  ...
  SELECT a.nom, a.espece, E.nom
  FROM animaux AS A
  INNER JOIN enclos_animaux AS EA
...
/* Story 3 */
  ...
  SELECT A.nom, A.espece, E.nom, E.niveau
  FROM animaux AS A
  INNER JOIN enclos_animaux AS EA
    ON EA.id_animal = A.id
  LEFT JOIN enclos AS E 
    ON  E.id = EA.id_enclot
  WHERE A.espece = "Orang-Outan"
  OR espece = "Lion"
  ORDER BY A.espece, E.niveau;

/* Story 4  */

  SELECT A.nom, A.espece, A.taille
  FROM animaux as A 
  INNER JOIN historique AS H
    ON A.id = H.id
...
/* Story 5 */
  ...
  SELECT E.nom, E.niveau, COUNT(id_animal) AS nombre_d_animaux
  FROM enclos AS E
  INNER JOIN enclos_animaux AS EA
  	 ON E.id = EA.id_enclot
  GROUP BY e.niveau
...
/* Story 6 */
  ...
  SELECT A.espece, A.sexe,  EA.date_entree, max(EA.date_entree)
  FROM animaux AS A 
  INNER JOIN enclos_animaux AS EA
    ON EA.id_animal = A.id
  WHERE espece = "faucon"
  ...
  SELECT A.espece, A.sexe,  EA.date_entree, min(EA.date_entree)
  FROM animaux AS A 
  INNER JOIN enclos_animaux AS EA
    ON EA.id_animal = A.id
  WHERE espece = "faucon"
...
/* Story 7 */
  ...
  SELECT A.nom A.espece EA.date_entree, TIMESTAMPDIFF(month,EA.date_entree, now()) AS ecart_de_mois
  FROM animaux A
  INNER JOIN enclos_animaux EA
    ON A.id = EA.id
  WHERE espece = "Chèvre"
  AND espece = "Pinguin";
...
/* Story 8 */
  ...
  SELECT A.nom, A.espece, sum(A.taille) AS taille_total
  FROM animaux AS A
...
/* Story 9 */
  ...
  SELECT year(H.date) AS année , sum(H.montant) AS montant
  FROM historique AS H 
  GROUP BY nuggets 
  ORDER BY H.date;
...
/* Story 10 */
  ...
  SELECT year(H.date_entree) AS date_entree , sum(H.id_animal) AS panyan
  FROM enclos_animaux AS H 
  GROUP BY date_entree
  ORDER BY date_entree

/* Story 11 */

 SELECT A.nom, E.nom, A.sexe, A.espece, min(EA.date_entree) AS date_de_naissance, TIMESTAMPDIFF(year, min(EA.date_entree), NOW()) as âge
  FROM animaux AS A
  INNER JOIN enclos_animaux AS EA 
...
/* Story 12 */
 ...
 SELECT A.nom, E.nom, A.sexe, A.espece, max(EA.date_entree) AS date_de_naissance, TIMESTAMPDIFF(year, max(EA.date_entree), NOW()) as âge
  FROM animaux AS A
  INNER JOIN enclos_animaux AS EA 
    ON A.id = EA.id_animal
  INNER JOIN enclos AS E 
...
/* Story 13 */
  ...
  SELECT A.*, TIMESTAMPDIFF(year, EA.date_entree, NOW()) as âge
  FROM animaux AS A
   INNER JOIN enclos_animaux AS EA 
    ON A.id = EA.id_animal
  INNER JOIN enclos AS E 
    ON E.id = EA.id_enclot
  WHERE E.nom = "VonRueden"
  AND YEAR (EA.date_entree) > 4 
  ORDER BY espece, âge;
